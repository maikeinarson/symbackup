	...
	********************************/
	...
	function makeLogoSmall() {
		...
		if (jQuery(window).width()>767) {
			...
			if (logo.data('mtop')== undefined) logo.data('mtop',parseInt(logo.css('marginTop'),0));
			if (logo.data('mbottom')== undefined) logo.data('mbottom',parseInt(logo.css('marginBottom')));
			TweenLite.to(logo,0.3,{scale:0.7, transformOrigin:"-20% 0%", marginTop:logo.data('mtop')*0.0, marginBottom:logo.data('mbottom')*0.5});

			var menu_top = logo.data('mtop')+((logo.height()*0.5)/2)-10;
			var menu_bottom = logo.data('mbottom')+((logo.height()*0.5)/2)-5;
			var header_search_top = logo.data('mtop')+((logo.height()*0.5)/2)-15;
			var header_language_top = logo.data('mtop')+((logo.height()*0.5)/2)-20;
			...
			jQuery('.navigation >ul>li>a').each(function() {
				...
				TweenLite.to(a,0.5,{paddingTop:newmt, paddingBottom:newmb});
			})
			...
			TweenLite.to(jQuery('.headerlanguage_selektor'),0.3,{top:header_language_top})
		}
	...
	function makeLogoNormal() {
		...
		if (jQuery(window).width()>767) {
				...
				// RESET LOGO
				var logo = jQuery('header .logo');
				TweenLite.to(logo,0.3,{scale:1.0, marginTop:logo.data('mtop'), marginBottom:logo.data('mbottom')});
				...
				var header_language_top = logo.data('mtop')+((logo.height())/2)-20;
				var hh = 0;
				jQuery('.navigation >ul>li>a').each(function() {
					var a = jQuery(this);
					if ((a.height()+menu_top+menu_bottom)>hh) hh = a.height()+menu_top+menu_bottom;
