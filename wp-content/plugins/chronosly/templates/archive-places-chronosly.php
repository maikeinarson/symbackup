...
$limit = (isset($_REQUEST["count"]) and $_REQUEST["count"])?$_REQUEST["count"]:$Post_Type_Chronosly->settings["chronosly_places_x_page"];
...
if(!isset($_REQUEST["shortcode"]) or !$_REQUEST["shortcode"])  {
    ...
    if(!has_action( 'posts_orderby', array("Post_Type_Chronosly_Places",'add_custom_places_orderby') )) add_action( 'posts_orderby', array("Post_Type_Chronosly",'add_custom_places_orderby') );
      $query = new WP_Query($args);
    if(has_action( 'posts_orderby', array("Post_Type_Chronosly_Places",'add_custom_places_orderby') )) remove_action( 'posts_orderby', array("Post_Type_Chronosly",'add_custom_places_orderby') );
    ?>
...
if(!isset($_REQUEST["shortcode"]) or !$_REQUEST["shortcode"] or (isset($_REQUEST["navigation"]) and $_REQUEST["navigation"])){
    ?>
                ...
                <div class="ch-header ch-<?php echo $Post_Type_Chronosly->settings["chronosly_titles_template_default"];?>" style="<?php echo $stilo;?>"><span class="title"><?php _e("Places", "chronosly"); ?></span>
...
echo "<div class='chronosly-content-block' style='".$stilo.";clear:both;'>";
                ...
                while ( $query->have_posts() ) {
                    ...
                    ob_start();
                    $Post_Type_Chronosly->template->print_template($xid, "dad9", "", "", "front");
                  ...
                  if(!isset($_REQUEST["shortcode"]) or $_REQUEST["pagination"]) {
                    if(!isset($_REQUEST["shortcode"])) {
                        ...
                        if(!$_REQUEST["ch_code"]) {
                            $_REQUEST["ch_code"] = array("type" => "place", "pagination" => 1, "count" => $limit);
