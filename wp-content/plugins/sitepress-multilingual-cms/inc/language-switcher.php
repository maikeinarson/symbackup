...
class SitePressLanguageSwitcher {
    ...
    public $color_schemes = array(
           'Symphoniker' => array(
                'font-current-normal' => 'rgba (255,255,255,0.5)',
                'font-current-hover' => 'rgba (255,255,255,1.0)',
                'background-current-normal' => 'rgba (0,0,0,0.5)',
                'background-current-hover' => 'rgba (0,0,0,1.0)',
                'font-other-normal' => 'rgba (255,255,255,0.5)',
                'font-other-hover' => 'rgba (255,255,255,1.0)',
		...
		<?php if ( !defined( 'ICL_DONT_LOAD_LANGUAGE_SELECTOR_CSS' ) || !ICL_DONT_LOAD_LANGUAGE_SELECTOR_CSS ): ?>
			...
			<div id="icl_lang_preview_config_wrapper" class="hidden">
				...
				<select id="icl_lang_sel_color_scheme" name="icl_lang_sel_color_scheme">
					...
					<option value="Symphoniker"><?php _e( 'Symphoniker', 'sitepress' ) ?>&nbsp;</option>
...
<?php
            ...
            foreach($this->settings['icl_lang_sel_config'] as $k=>$v){
                switch($k){
                    ...
                    case 'border':
                        //if($v != $this->widget_css_defaults[$k])
                        if ($list)
                             echo '#lang_sel_list a, #lang_sel_list a:visited{border-color:'.$v.';} #lang_sel_list  ul{border-top:0px solid '.$v.';}';
                        else
    ...
    function wp_nav_menu_items_filter($items, $args){
	    ...
	    if ( $abs_menu_id == $settings_menu_id  || false === $abs_menu_id ) {
            ...
            $items .= '<a class="menu-link" href="#" onclick="return false">';
            ...
            if(!empty($languages)){
                foreach($languages as $lang){
                    ...
                    $sub_items .= '<a class="menu-link" href="'.$lang['url'].'">';
